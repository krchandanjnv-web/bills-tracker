...
class FooterCanvas(rl_canvas.Canvas):
    """Adds a footer with page numbers to every PDF page."""
    ...
    def save(self):
        ...
        for i, state in enumerate(self._saved_page_states):
            ...
            self._draw_footer(page_num=i + 1, page_count=num_pages)
            super().showPage()
        super().save()
    ...
    def _draw_footer(self, page_num, page_count):
        ...
        self.drawRightString(
            A4[0] - 15*mm, 8*mm,
            f"Generated: {datetime.now().strftime('%d %b %Y')}  |  Page {page_num} of {page_count}"
        )
...
# ═══════════════════════════════════════════════════════════════════════════════
# HISTORY  (filtered PDF export + global search)
# ═══════════════════════════════════════════════════════════════════════════════
def show_history():
    df = db.get_user_data(st.session_state.username)

    # ── Header row: Title left, Search + PDF right ────────────────────────────
    hc, sc, bc = st.columns([2.5, 2, 1])
    with hc:
        ...
        <div style="color:#718096;font-size:0.85rem;margin-bottom:1.5rem;">Full history · search, filter, edit or delete</div>""", unsafe_allow_html=True)
    ...
    # Apply global search (description OR amount match, case-insensitive)
    if search_query.strip():
        q = search_query.strip().lower()
        desc_match   = filtered["Description"].astype(str).str.lower().str.contains(q, na=False)
        amount_match = filtered["Amount"].astype(str).str.contains(q, na=False)
        filtered = filtered[desc_match | amount_match]
    ...
    if search_query.strip(): parts.append(f'Search: "{search_query.strip()}"'  )
    filter_label = " · ".join(parts) if parts else "All Transactions"

    # PDF button — exports ONLY currently visible rows
    with bc:
        st.markdown("<div style='height:0.3rem'></div>", unsafe_allow_html=True)
